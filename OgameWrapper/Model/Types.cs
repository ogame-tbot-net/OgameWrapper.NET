...
namespace OgameWrapper.Model
{
    ...
    public class Celestial
    {
        ...
        public async Task GetTechs(OgameWrapperClient client)
        {
        ...
        public async Task GetBuildings(OgameWrapperClient client)
        {
            var techs = await client.GetTechs(this);
            Buildings = techs.Buildings;
        }

        public async Task GetFacilities(OgameWrapperClient client)
        {
            var techs = await client.GetTechs(this);
            Facilities = techs.Facilities;
        }

        public async Task GetShips(OgameWrapperClient client)
        {
            var techs = await client.GetTechs(this);
            Ships = techs.Ships;// TOFIX : always LargeCargo (5)
        }

        public async Task GetDefences(OgameWrapperClient client)
        {
            var techs = await client.GetTechs(this);
            Defences = techs.Defences;
        }

        public async Task GetResources(OgameWrapperClient client)
        {
            var resources = await client.GetResources(this);
            Resources = resources; // TOFIX : always null
        }

        public async Task GetResourcesProduction(OgameWrapperClient client)
        {
            var resourcesProduction = await client.GetResourcesProduction(this);
            ResourcesProduction = resourcesProduction; // TOFIX : always null
        }

        public async Task GetResourceSettings(OgameWrapperClient client)
        {
            var resourcesSettings = await client.GetResourceSettings(this);
            ResourceSettings = resourcesSettings; // TOFIX : always null
        }
    }
